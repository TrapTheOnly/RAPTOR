...
def create_or_update_pentest_data(record_id):
    ...
    try:
        ...
        if 'report' in request.files:
            ...
            if report_file.filename != '' and report_file.filename.lower().endswith('.pdf'):
                try:
                    ...
                    relative_path = save_report(record_id, file_data)
                except Exception as e:
        ...
        pentest_data = {
            ...
            'vulnerable': data.get('vulnerable', 0) if data else 0,
            'tested_by': data.get('tested_by', '') if data else '',
        ...
        # Check if data for the record ID exists.
        existing_data = get_pentest_data_internal(record_id)

        if existing_data:
            # Filter out None values from pentest_data
            ...
            c.execute("""
                INSERT INTO pentest_data (record_id, dns_name, ip_address, source, report_file, vulnerable,
                                         tested_by, test_start_date, test_end_date, vulnerability_fixed, service_desk_link)
