...
def ldap_authenticate(username, password):
    ...
    from .admin import admin_login  # avoid circular import
...
def search_ldap_users(query, page_size=500):
    ...
    try:
        ...
        with Connection(server, user=LDAP_USER, password=LDAP_PASS, auto_bind=True) as conn:
            logger.info("LDAP connection successful. Search query: %s", query)
            search_filter, attributes = (
                f"(|(sAMAccountName=*{query}*)(mail=*{query}*))",
                ["name", "sAMAccountName", "distinguishedName", "mail"],
            )
            ...
            while True:
                ...
                for entry in conn.entries:
                    entry_count += 1
                    logger.debug("Processing entry %s", entry_count)
                ...
                logger.info("Retrieved %s entries in the current page", len(results))
                cookie = (
                ...
                if not cookie:
                    break
            logger.info("Total entries retrieved: %s", len(total_entries))
            return total_entries
    except Exception as e:
        logger.error("Error searching LDAP: %s", e)
